function Get-DevEnv {
    ...
    if(!(Test-Admin)) {
        ...
        $command = "-ExecutionPolicy bypass -noexit -command . '$(${function:Get-DevEnv}.File)';Get-DevEnv $($args)"
        Start-Process powershell -verb runas -argumentlist $command
        return
    }

    Write-Output "Welcome to the Windows Development Environment installer!"
    if(Check-Chocolatey -Force:$Force){
        Write-Output "Chocolatey installed, Installing DevOps Tools."
        $version = choco -v
         Write-Output $version
        $tools = @("cmder", "git.install", "git-credential-winstore", "virtualbox", "vagrant")
        foreach ($element in $tools) {
            Write-Output $element
	        $command = "cinst -y $element"
            Write-Output $command
            try{
                Invoke-Expression $command
                $Message = "$element Installation completed"
            }
            catch{
                Invoke-Expression $command
                $Message = "$element Installation failed!"
            }
            Write-Host $Message
        }
